function chplot(firstfile,lastfile,rseg,plotver,chnum,psdplt,xver)
% CHPLOT(firstfile,lastfile)
%
% WORK IN-PROGRESS --> MERGING WITH WAV2MAT.m to be more general
%
% INPUT:
%
% firstfile    the running number of the first file, e.g. 0, 1, 99, 362
% lastfile     the running number of the last file
% rseg         size of segment length in seconds [default: 5]
% plotver      switch (1 or 0) to turn on/off plotting feature [default: 1]
% chnum        number of channels [default: 4]
% psdplot      switch (1 or 0) to turn on/off psd plot [default: 0]
% xver         switch (1 or 0) to turn on/off cross-correlation feature [default: 0]
%
% OUTPUT:
%  
...
% Last modified by tschuh@princeton.edu, 02/10/2022

% By default, 4 channels
defval('chnum',4)
...
for file = firstfile:lastfile
  ...
  if exist(sname) ~= 2
    % Read the data
    fid = fopen(fname);
    FourChan = reshape(fread(fid,inf,'int16'),chnum,[]);
    fclose(fid);
  ...
  if plotver == 1
      % total number of plotting columns
      cols = (2+psdplt);
      spnum = 2*chnum*cols;
      ...
      for i=1:spnum
          ...
          if psdplt == 1
              % plot psd
              subplot(chnum,cols,i)

              % plot raw data
              subplot(chnum,cols,i+1)

              % plot subsampled data
              subplot(chnum,cols,i+2)
          else
              % plot raw data
              subplot(chnum,cols,i)
      ...
      tt=supertit(ah([1 2]),sprintf('Ship Data from %s to %s',d.t(1),d.t(end)));
